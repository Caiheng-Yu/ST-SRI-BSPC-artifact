# R008 补充相位曲线采集（打乱模型检查点：trained/reinit/param_scramble，零模型来源）
param(
...
$RunLog = Join-Path $LogDirectory 'r008_phase_curves_shuffled.log'
$StatusPath = Join-Path $ResearchRoot 'results\bspc_revision_v2\r008_phase_curves_shuffled\r008_phase_shuffled_status.json'

New-Item -ItemType Directory -Force -Path $LogDirectory, (Join-Path $ResearchRoot 'results\bspc_revision_v2\r008_phase_curves_shuffled') | Out-Null

function Write-Status {
    param([string]$State, [hashtable]$Extra = @{})
    $Payload = @{
        state = $State
        run_name = 'r008_phase_curves_shuffled'
        process_id = $PID
...
try {
    for ($Attempt = 1; $Attempt -le $MaxAttemptsPerRun; $Attempt++) {
        ...
        "[$(Get-Date -Format o)] r008_phase_curves_shuffled attempt $Attempt/$MaxAttemptsPerRun" |
            Tee-Object -FilePath $RunLog -Append

        & $Python -u $Script `
            --all-subjects `
            --checkpoint-dir checkpoints_bspc_v2\r006_label_shuffle `
            --output-dir results\bspc_revision_v2\r008_phase_curves_shuffled `
        ...
        "[$(Get-Date -Format o)] r008_phase_curves_shuffled failed with exit code $ExitCode" |
            Tee-Object -FilePath $RunLog -Append
