# R006c：标签打乱检查点的曲线采集（训练/重初始化/参数置乱三变体）
param(
...
$RunLog = Join-Path $LogDirectory 'r006c_collect_shuffled.log'
$StatusPath = Join-Path $ResearchRoot 'results\bspc_revision_v2\r006_curves_shuffled\r006c_status.json'

New-Item -ItemType Directory -Force -Path $LogDirectory, (Join-Path $ResearchRoot 'results\bspc_revision_v2\r006_curves_shuffled') | Out-Null

function Write-Status {
    param([string]$State, [hashtable]$Extra = @{})
    $Payload = @{
        state = $State
        run_name = 'r006c_collect_shuffled'
        process_id = $PID
...
try {
    for ($Attempt = 1; $Attempt -le $MaxAttemptsPerRun; $Attempt++) {
        ...
        "[$(Get-Date -Format o)] r006c_collect_shuffled attempt $Attempt/$MaxAttemptsPerRun" |
            Tee-Object -FilePath $RunLog -Append

        & $Python -u $Script `
            --all-subjects `
            --checkpoint-dir checkpoints_bspc_v2\r006_label_shuffle `
            --output-dir results\bspc_revision_v2\r006_curves_shuffled `
            --device cuda `
            --threads 2 2>&1 |
            Tee-Object -FilePath $RunLog -Append
        ...
        "[$(Get-Date -Format o)] r006c_collect_shuffled failed with exit code $ExitCode" |
            Tee-Object -FilePath $RunLog -Append
