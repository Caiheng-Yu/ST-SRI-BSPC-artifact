# R008 补充：true-mode（真实类别目标）onset 曲线采集——真模型三变体
param(
...
$RunLog = Join-Path $LogDirectory 'r008_true_curves.log'
$StatusPath = Join-Path $ResearchRoot 'results\bspc_revision_v2\r006_curves_true\r008_true_status.json'

New-Item -ItemType Directory -Force -Path $LogDirectory, (Join-Path $ResearchRoot 'results\bspc_revision_v2\r006_curves_true') | Out-Null

function Write-Status {
    param([string]$State, [hashtable]$Extra = @{})
    $Payload = @{
        state = $State
        run_name = 'r008_true_curves'
        process_id = $PID
...
try {
    for ($Attempt = 1; $Attempt -le $MaxAttemptsPerRun; $Attempt++) {
        ...
        "[$(Get-Date -Format o)] r008_true_curves attempt $Attempt/$MaxAttemptsPerRun" |
            Tee-Object -FilePath $RunLog -Append

        & $Python -u $Script `
            --all-subjects `
            --checkpoint-dir checkpoints_bspc_v2\r005b_balanced_full_seed20260815 `
            --output-dir results\bspc_revision_v2\r006_curves_true `
            --target-mode true `
        ...
        "[$(Get-Date -Format o)] r008_true_curves failed with exit code $ExitCode" |
            Tee-Object -FilePath $RunLog -Append
